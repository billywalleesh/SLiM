...
GRID="${GRID:-${OUTBASE}/manifests/parameter_grid_ramp_v1_${timestamp}.tsv}"

mkdir -p "${OUTBASE}/manifests" "${OUTBASE}/slurm"

bash "${SCRIPT_DIR}/make_ramp_v1_parameter_grid.sh" \
	"${REPLICATES}" "${GRID}"
...
job_id="$(
	sbatch \
		...
		"${SCRIPT_DIR}/run_ramp_v1_array.sh"
)"
