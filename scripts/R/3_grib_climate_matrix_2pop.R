### Convert raw GRIB temperature into a year-by-population climate anomaly matrix.
###
### Biological story:
### Each population has its own historical baseline climate.
### A climate anomaly asks: how far is this year from that population's baseline?
###
### Output for SLiM:
###   grib_climate_matrix_2pop.csv
###     rows    = climate years
###     columns = populations p0, p1, p2, ...
###     values  = degrees C away from each population's own baseline
###
###   grib_climate_years_2pop.csv
###     one year per row, matching the rows of the anomaly matrix
...
population_file <- file.path(input_directory, "population_locations/grib_population_locations_2pop.csv")

climate_matrix_file <- file.path(input_directory, "climate_matrix/grib_climate_matrix_2pop.csv")
climate_years_file <- file.path(input_directory, "climate_matrix/grib_climate_years_2pop.csv")
metadata_file <- file.path(input_directory, "metadata/grib_climate_matrix_2pop_metadata.csv")
baseline_file <- file.path(input_directory, "metadata/grib_climate_matrix_2pop_baseline_metadata.csv")

### The baseline should represent the climate each population is historically adapted to.
### Earlier climate years can still be used in the time series; this only defines zero anomaly.
...
if (!all(required_columns %in% names(population_locations)))
	stop("grib_population_locations_2pop.csv needs columns: pop, lon, lat")
...
climate_anomaly_c <- sweep(annual_temperature_c, 2, baseline_temperature_c, "-")
colnames(climate_anomaly_c) <- paste0("p", population_locations$pop)

write.table(
	round(climate_anomaly_c, 4),
...
message("Wrote anomaly metadata to: ", metadata_file)
